...
::: {.callout-note}
The canonical FSM — including the full state diagram, wakeup-cause dispatch (`CHECK_WAKEUP`, `READ_SWITCH`), and LED table — is defined in [System Architecture](02-architecture.qmd#station-finite-state-machine-fsm). This section covers the **implementation perspective** only: what each state does in code and how boot priority is resolved.
:::
...
| `SERVER` | Mode switch in SERVER position (`EXT1` wakeup or power-on) | Start Wi-Fi AP → serve web UI → no measurements |
