...
1. **`pinMode(SD_OFF_PIN, OUTPUT); digitalWrite(SD_OFF_PIN, HIGH);`**: We first power on the SD card module by pulling the OFF pin HIGH. This gives the module power so we can initialize it.
2. **`SD.begin(SD_CS_PIN)`**: Initializes communication with the SD card using the standard SPI pins, but specifying our custom Chip Select (CS) pin (Pin 5).
3. **`appendFile(...)`**: A helper function that opens the file in `FILE_APPEND` mode. This ensures that new data is added to the end of the file without overwriting previous measurements.
4. **`SD.end(); digitalWrite(SD_OFF_PIN, LOW);`**: After writing, we end the SD SPI connection and pull the OFF pin LOW to completely shut off power to the module, saving battery during the 5-second delay.
...
With data acquisition and persistent storage now implemented, we have a fully functional baseline for our weather station! However, our timestamps are currently just the number of milliseconds since the device booted. In the next chapter, we will integrate a Real-Time Clock (RTC) to provide accurate, real-world timestamps for our data!
