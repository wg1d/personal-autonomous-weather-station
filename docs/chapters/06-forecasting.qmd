...
> Status: 🔴 Planned (Phase 3 — basic LSTM, Phase 8 — full retrain + watering model)
...
::: {.callout-important}
**Data requirements differ between models.** The basic LSTM (Phase 3) trains on OpenMeteo historical data — no local sensor data required to start. The full retrain and watering model (Phase 8) require local sensor data collected after outdoor deployment. The watering model specifically needs soil moisture, rain, and wind data (Phases 5–7).
:::
...
- **Fast inference**: predictions take milliseconds on a low-power SBC (Odroid C4 / Raspberry Pi)
...
| `wind_avg_24h` | Mean wind speed m/s (Phase 7+) | Evapotranspiration factor |
...
### Deployment on SBC (Odroid C4 / Raspberry Pi)

| Task | Feasibility |
|------|-------------|
| LSTM inference (small model <100k params) | ✅ <100 ms per prediction |
| LSTM training from scratch | 🟡 Hours — prefer laptop |
| Transfer learning fine-tuning | ✅ Minutes on SBC CPU |
| TFLite / ONNX export + inference | ✅ Recommended for production |

**Workflow:** train on laptop → export to ONNX or TFLite → deploy to SBC for serving.
...
| Train on SBC from scratch | Slow. Train on laptop, deploy to SBC. |
